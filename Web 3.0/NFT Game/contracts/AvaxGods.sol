...
//isPlayer
//isPlayer which accepts a parameter of type address which is called addr
//it is a public function which means we can call it from outside and it will return a boolean.
  ...
  function getPlayer(address addr) public view returns (Player memory) {
    //When you see require in solidity, it is some kind of check a statement
    //that can throw us out of the function call.
  ...
  function createBattle(string memory _name) external returns (Battle memory) {
    ...
    Battle memory _battle = Battle(
      ...
      _name, // Battle name and we are creating addresses.
      [msg.sender, address(0)], // player addresses; player 2 empty meaning it doesnt exist until the player joins battle 
      [0, 0], // the computer is waiting for each player to make a move. (set to 0 first)
